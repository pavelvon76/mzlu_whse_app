...
export const ProgressUI: React.FC<iProps> = (props: iProps) => {
  ...
  // useEffect to samé jako EventListener, jen je pověšený na proměnnou
  React.useEffect(() => {
  ...
  function fireScanned (param:any) {
    const rawContent = {
      outPcnt: param
    }
    let event = new CustomEvent(ON_SCANNED, { detail: rawContent });
    window.dispatchEvent(event);
  }
